...
let gCHeight = parseInt(
	window.getComputedStyle(gameContainer).getPropertyValue("height")
);

diagonal.style.width = `${
	Math.round(Math.sqrt(gCHeight ** 2 + gCHeight ** 2)) - 32
}px`;
...
function checkWinner(playerEntry) {
	if (
		gameArray[0][0] === playerEntry &&
		gameArray[0][1] === playerEntry &&
		gameArray[0][2] === playerEntry
	) {
		return "h1";
	}
	if (
		gameArray[1][0] === playerEntry &&
		gameArray[1][1] === playerEntry &&
		gameArray[1][2] === playerEntry
	) {
		return "h2";
	}
	if (
		gameArray[2][0] === playerEntry &&
		gameArray[2][1] === playerEntry &&
		gameArray[2][2] === playerEntry
	) {
		return "h3";
	}
	if (
...
function victory(keyword, playerCode) {
	if (keyword === "no winner") {
		...
		if (keyword === "h1") {
			...
			leftRight.style.top = `${
				(parseInt(
					window.getComputedStyle(boxes[0]).getPropertyValue("height")
				) -
					10) /
				2
			}px`;
			...
			leftRight.style.top = `${
				(parseInt(
					...
					10) /
					2 +
				parseInt(window.getComputedStyle(boxes[0]).getPropertyValue("height"))
			}px`;
		} else if (keyword === "h3") {
			leftRight.classList.remove("hidden");
			leftRight.style.top = `${
			...
			diagonal.classList.remove("hidden");
			diagonal.style.transform = "translate(-49%, 0) rotate(135deg)";
		}
	}
}
